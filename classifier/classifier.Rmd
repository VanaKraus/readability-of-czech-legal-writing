...
plot_outlier <- function(doc_name, variable_importances, dataset) {
  important_variables <- sort(variable_importances, decreasing = TRUE) %>% head(n = 16)
...
for (doc in mismatches_all$highest_deviations) {
  ...
  cat(paste(
    doc, "/", doc_row["subcorpus"][[1]],
    "/ dev:", doc_row["abs_dev"][[1]] %>% round(3), "\n"
  ))
  print(plot_outlier(doc, variable_importances, data_clean))
}
```
