...
# )
colsids <- c("all", "readforms")
colsets <- list(columns_all, columns_readabilty_forms)
```
...
mismatches_all$plot + theme_bw() + labs(y = "true class")
...
for (doc in mismatches_all$highest_deviations) {
  ...
  plt <- plot_outlier(doc, variable_importances, data_clean) + theme_bw()
  print(plt)
  ggsave(paste(c("out_", doc_row["KUK_ID"][[1]], ".pdf"), collapse = ""), plt)
}
```
