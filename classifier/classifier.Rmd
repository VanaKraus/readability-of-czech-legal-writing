...
```{r}
data_clean_good <- data_clean %>% filter(class == "good")
data_clean_bad <- data_clean %>% filter(class == "bad")
...
feature_importances <- tibble(feat_name = character(), p_value = numeric())
for (i in 6:84) {
...
var_selection <- importances_ranked %>%
  ...
  mutate(
    ...
    using_p_value = p_value <= 0.05
  )

var_selection %>%
  select(using_classifiers, using_p_value) %>%
  table(useNA = "ifany")

var_selection %>%
  filter(using_classifiers & !using_p_value) %>%
  pull(Variable)

cor(log10(var_selection$p_value), var_selection$mean_quantile)
