...
```{r}
meas <- read_csv("../measurements/measurements_2025-12-02.csv") %>%
...
data_clean <- data_no_nas %>%
  ...
  select(!c(RuleIncompleteConstruction)) %>%
...
for (variab in names(data_clean)[.firstnonmetacolumn:ncol(data_clean)]) {
  ...
  variable_polarity[variab] <- positive
...
intervals %>% write_csv("intervals_2025-12-02.csv")
...
for (variab in names(data_clean)[.firstnonmetacolumn:ncol(data_clean)]) {
  cat("boxplot", variab, "\n")
  print(
    data_clean %>% ggplot(aes(x = class, y = !!sym(variab))) +
      geom_boxplot()
  )
}
```
...
```{r}
parameter_variations <- names(data_clean)[.firstnonmetacolumn:ncol(data_clean)][
  str_ends(names(data_clean)[.firstnonmetacolumn:ncol(data_clean)], "\\.v")
]
parameters <- str_remove(parameter_variations, "\\.v")

data_comb <- data_clean %>% select(KUK_ID, class)

for (p in parameters) {
  combined_df <- data.frame(
    data_clean[[p]] +
      data_clean[[paste0(c(p, ".v"), collapse = "")]] *
        if_else(variable_polarity[p], 0, 1)
  )
  names(combined_df) <- paste0(c(p, ".comb"), collapse = "")

  data_comb <- data_comb %>% bind_cols(combined_df)
}

intervals_comb <- tibble(
  feat = character(),
  type = character(),
  minimum = numeric(),
  maximum = numeric(),
...
for (variab in names(data_comb)[3:ncol(data_comb)]) {
  cat(variab, "\n")

  distr_good <- data_comb %>%
    filter(class == "good") %>%
    pull({{ variab }})
  distr_bad <- data_comb %>%
    filter(class == "bad") %>%
  ...
  if (
    (quant_good[1] < quant_bad[1] & quant_good[2] > quant_bad[2]) |
      (quant_good[1] > quant_bad[1] & quant_good[2] < quant_bad[2])) {
    cat(variab, "not determinative\n")
  } else if (variable_polarity[str_remove(variab, "\\.comb")]) {
    intervals_comb <- intervals_comb %>%
      add_row(
        feat = variab,
        type = "bad",
        minimum = -Inf,
