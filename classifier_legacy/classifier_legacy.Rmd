...
# data_clean_long <- data_clean %>% pivot_longer(!c(
#   KUK_ID,
#   strata,
#   class,
#   FileName,
...
feature_importances <- tibble(
  feat_name = character(), p_value_wilcox = numeric(), p_value_glm = numeric()
)
...
for (i in 6:84) {
  ...
  p_value_wilcox <- wilcox.test(
    data_clean_bad[i] %>% pull(), data_clean_good[i] %>% pull()
  )$p.value
  ...
  p_value_glm <- glm_coefficients[row_index, 4]

  feature_importances <- feature_importances %>%
    add_row(feat_name = fname, p_value_wilcox = p_value_wilcox, p_value_glm = p_value_glm)
}
feature_importances
```
...
Remove `fkgl` from the formulas as it's very similar to `fre`.
...
get_mismatch_details <- function(lfit, data_orig) {
  ...
  return(list(deviations = deviations, highest_deviations = highest_deviation_names))
}

plot_outlier <- function(doc_name, final_fit, dataset) {
  important_variables <- get_vi(final_fit) %>%
    head(n = 6) %>%
    filter(Importance > 0) %>%
    pull(Variable)

  dmut <- dataset %>%
    select(KUK_ID, FileName, class, all_of(important_variables)) %>%
    mutate(across(all_of(important_variables), ~ scale(.x))) %>%
    pivot_longer(
      all_of(important_variables),
...
Variation inflation (>5 is considered bad; sometimes the threshold is >10).
...
lasso_all_devs <- lfit_lasso_all %>% get_mismatch_details(data)
# lfit_lasso_all %>%
...
lasso_notl_devs <- lfit_lasso_notl %>% get_mismatch_details(data)
# lfit_lasso_notl %>%
#   lasso_get_coefficients() %>%
#   print(n = 100)
```
...
lasso_iac_devs <- lfit_lasso_iac %>% get_mismatch_details(data)
# lfit_lasso_iac %>%
#   lasso_get_coefficients() %>%
#   print(n = 100)
```
...
lasso_counts_devs <- lfit_lasso_counts %>% get_mismatch_details(data)
# lfit_lasso_counts %>%
#   lasso_get_coefficients() %>%
#   print(n = 100)
```
...
rf_all_devs <- lfit_rf_all %>% get_mismatch_details(data)
```
...
rf_notl_devs <- lfit_rf_notl %>% get_mismatch_details(data)
```
...
rf_iac_devs <- lfit_rf_iac %>% get_mismatch_details(data)
```
...
rf_counts_devs <- lfit_rf_counts %>% get_mismatch_details(data)
```
...
var_selection <- importances_ranked %>%
  left_join(
    feature_importances %>% rename(Variable = feat_name),
    by = "Variable"
  ) %>%
  mutate(
    using_classifiers = omissions < 3 | mean_quantile < 0.5,
    using_p_value_wilcox = p_value_wilcox <= 0.05,
    using_p_value_glm = p_value_glm <= 0.05
  )

var_selection %>%
  select(using_classifiers, using_p_value_wilcox) %>%
  table(useNA = "ifany")

var_selection %>%
  select(using_p_value_glm, using_p_value_wilcox) %>%
  table(useNA = "ifany")

var_selection %>%
  select(using_classifiers, using_p_value_glm) %>%
  table(useNA = "ifany")

var_selection %>%
  filter(using_classifiers & !using_p_value_wilcox) %>%
  pull(Variable)

var_selection %>%
  filter(using_p_value_glm & !using_p_value_wilcox) %>%
  pull(Variable)
...
cor(
  log10(var_selection$p_value_wilcox),
  log10(var_selection$p_value_glm),
  var_selection$mean_quantile
)
plot(log10(var_selection$p_value), var_selection$mean_quantile)

var_selection %>%
  ggplot(aes(x = log10(p_value), y = mean_quantile, color = using_classifiers & using_p_value)) +
  geom_point() +
  labs(color = "use in EFA")
```

It may be more intuitive to stick to the p-values **produced by GLMs** instead of the ones of the Wilcoxon test, beacuse GLM p-values are easier to interpret as indicating the "usefulness" of the associated predictor, whereas the Wilcoxon test tests for a significant difference in medians of the two distributions, which is very similar (and gives similar results), but the intuition behind the logic isn't as clear. Afterall, we're trying to select useful predictors.
...
```{r}
plot_outlier("orig_Jak uspořádat shromáždění", lfit_lasso_all, data_clean)
```
...
```{r}
plot_outlier("orig_Zastupitelstvo_o čem a jak rozhoduje", lfit_lasso_all, data_clean)
```
...
```{r}
plot_outlier("Mestsky_urad_usneseni_-_slouceni_pred", lfit_lasso_all, data_clean)
```
...
```{r}
plot_outlier("2A_dokument_puvodni_vyzva_k_zaplaceni_SOP_a_k_doplneni_kast_pouceni", lfit_lasso_all, data_clean)
```
...
```{r}
plot_outlier("orig_Jak namítat podjatost_final", lfit_lasso_all, data_clean)
```
...
```{r}
plot_outlier("Mestsky_urad_Vyzva_k_zaplaceni_nakladu_rizeni_pred", lfit_lasso_iac, data_clean)
```

### FrBo / orig_Jak probíhá správní řízení
...
```{r}
plot_outlier("orig_Jak probíhá správní řízení", lfit_lasso_iac, data_clean)
```
...
```{r}
plot_outlier("Reakce_na_dopis_pracovni", lfit_lasso_counts, data_clean)
```
...
```{r}
plot_outlier("Reakce_na_dopis_rev", lfit_lasso_counts, data_clean)
```

### FrBo / orig_Kterých řízení se může váš spolek účastnit_FINAL
...
```{r}
plot_outlier("orig_Kterých řízení se může váš spolek účastnit_FINAL", lfit_lasso_counts, data_clean)
```

### FrBo / red_Certifikáty autorizovaných inspektorů
...
```{r}
plot_outlier("red_Certifikáty autorizovaných inspektorů", lfit_lasso_counts, data_clean)
```
...
```{r}
plot_outlier("1A_dokument_puvodni_ustanoven_zastupce_vyzva_k_doplneni_kast_pouceni", lfit_lasso_counts, data_clean)
```
...
```{r}
plot_outlier("orig_Jak uspořádat shromáždění", lfit_rf_all, data_clean)
```
...
```{r}
plot_outlier("33 Cdo 30_2024", lfit_rf_all, data_clean)
```
...
```{r}
plot_outlier("11_vizum_pred", lfit_rf_all, data_clean)
```
...
```{r}
plot_outlier("orig_Kterých řízení se může váš spolek účastnit_FINAL", lfit_rf_all, data_clean)
```
...
```{r}
plot_outlier("orig_lhuty_v_jednani_s_urady_a_soudy", lfit_rf_all, data_clean)
```
...
```{r}
plot_outlier("orig_Zastupitelstvo_o čem a jak rozhoduje", lfit_rf_all, data_clean)
```
...
```{r}
plot_outlier("orig_Jak probíhá správní řízení", lfit_rf_all, data_clean)
```
...
```{r}
plot_outlier("orig_Jak namítat podjatost_final", lfit_rf_all, data_clean)
```
...
```{r}
plot_outlier("142", lfit_rf_iac, data_clean)
```
...
```{r}
plot_outlier("64", lfit_rf_counts, data_clean)
```
...
```{r}
plot_outlier("red_Certifikáty autorizovaných inspektorů", lfit_rf_counts, data_clean)
```
...
```{r}
plot_outlier("2A_dokument_puvodni_vyzva_k_zaplaceni_SOP_a_k_doplneni_kast_pouceni", lfit_rf_counts, data_clean)
```
...
```{r}
plot_outlier("orig_provokace_korupcniho_jednani", lfit_rf_counts, data_clean)
```

## Summary

All outliers (*notl* models excluded since they're the same as *all*):

```{r}
table(c(
  lasso_all_devs$highest_deviations,
  lasso_iac_devs$highest_deviations,
  lasso_counts_devs$highest_deviations,
  rf_all_devs$highest_deviations,
  rf_iac_devs$highest_deviations,
  rf_counts_devs$highest_deviations
